...
export function RideOfferPopup({ offer, onAccept, onReject, onDismiss }) {
  ...
  useEffect(() => {
    if (activeOffer?.driverName) {
      const fetchDriver = async () => {
        const { data } = await supabase
          .from('profiles')
          .select('profile_data')
          .eq('name', activeOffer.driverName)
          .single();
  ...
  const driverName = activeOffer?.driverName || 'Someone';
  const driverAvatar = activeOffer?.driverAvatarUrl || driverProfile?.avatarUrl;
  const message = activeOffer?.driverMessage || "Hey there! I could pick you up, would that work?";
  const pickupAddr = activeOffer?.pickup || 'Kerklaan 15';
  const dropoffAddr = activeOffer?.destination || 'Noordereinde 42';
  const requestedTime = activeOffer?.timeframe || '14:00-17:00';
    ...
    <AnimatePresence>
      {offer && (
        ...
        >
          ...
          >
            <motion.button 
              className="offer-close-x" 
              onClick={() => onDismiss ? onDismiss(activeOffer.id) : onReject(activeOffer.id)}
              whileTap={{ scale: 0.9 }}
            ...
            <div className="gr-actions" style={{ marginTop: '16px', justifyContent: 'center' }}>
              <motion.button 
                className="gr-btn offer-btn--reject" 
                onClick={() => onReject(activeOffer.id)}
                whileTap={{ scale: 0.95 }}
